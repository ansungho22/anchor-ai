...
- **문서 파일명 지정 규칙 (Strict Naming Convention):** 에이전트가 문서를 작성할 때는 반드시 기준 이름 뒤에 **기능명(Feature Name)**을 덧붙여야 합니다. (소문자 및 하이픈 권장)
  ...
  - **ADR (결정 기록):** `docs/adr/001-[결정사항].md`, `docs/adr/002-[결정사항].md` (순차적 번호 부여)
...
* **[Phase 2] 아키텍처 및 디자인 설계 (`architect`, `ui_ux_designer`, `dba`)**
  ...
  - **목표 산출물:** `docs/specs/` 내 기술 스펙, `docs/adr/` 내 주요 아키텍처 결정 기록(ADR), 디자인 문서 완료 (반드시 **Mermaid를 활용한 시퀀스 다이어그램 및 구조도** 포함)
...
## 7. Restricted Area Policy (템플릿 접근 금지 영역)
- **Rule:** `docs/templates/` 디렉토리는 시스템의 근간이 되는 템플릿 파일들을 보관하는 **'절대 접근 금지(Read-Only)'** 영역입니다.
- **Action:** 어떠한 에이전트도 이 폴더 내부의 템플릿 파일(`.md`) 내용을 수정, 덮어쓰기, 삭제해서는 안 됩니다. 에이전트는 오직 문서를 새로 작성할 때 템플릿의 내용을 읽어오기(Read) 위한 용도로만 이 폴더에 접근해야 합니다.
