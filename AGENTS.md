...
## 7. Clean Workspace Policy (임시 파일 정리 규칙)
- **Rule:** 작업 중 디버깅이나 테스트를 위해 임의로 생성한 스크래치 파일(일회성 스크립트, 더미 데이터, 로그 등)은 작업 완료 즉시 삭제되어야 합니다.
- **Action:** 태스크를 마치고 다음 단계로 넘어가기 전, 본인이 생성한 모든 일회성 임시 파일들을 스스로 찾아 지우고(cleanup) 작업 공간을 항상 청결하게 유지하십시오.
